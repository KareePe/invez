...
$base_url = 'properties' . ($filter !== 'all' ? '?status=' . $filter : '');

if ($filter === 'pending') {
    $total = (int)db()->query("SELECT COUNT(*) FROM properties WHERE approval_status='pending'")->fetchColumn();
    $properties = db()->query(
    ...
    $total = (int)db()->query("SELECT COUNT(*) FROM properties WHERE approval_status='approved' OR submitted_by IS NULL")->fetchColumn();
    $properties = db()->query(
        "SELECT p.id, p.title, p.category, p.price_display, p.status, p.is_active, p.approval_status,
                (SELECT filename FROM property_images WHERE property_id=p.id ORDER BY sort_order ASC LIMIT 1) AS thumb
         FROM properties p
        ...
        <table class="w-full text-sm min-w-[640px]">
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-5 py-3 text-[#c9a96e] text-sm whitespace-nowrap"><?= htmlspecialchars($p['price_display'] ?? '-') ?></td>
                    <?php if ($filter === 'pending'): ?>
                    <td class="px-5 py-3 text-xs text-gray-500 whitespace-nowrap"><?= htmlspecialchars(($p['first_name'] ?? '') . ' ' . ($p['last_name'] ?? '')) ?></td>
                    <?php else: ?>
                    ...
                    <td class="px-5 py-3 whitespace-nowrap">
                        <div class="flex items-center gap-2">
                            ...
                            <form method="POST" data-loading="overlay" action="property-delete" data-confirm="ลบทรัพย์สินนี้?">
                                <input type="hidden" name="id" value="<?= $p['id'] ?>">
                                <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">
