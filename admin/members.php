...
$base_url = 'members' . ($filter !== 'all' ? '?status=' . $filter : '');
...
if ($filter === 'all') {
    ...
    $stmt = db()->prepare("SELECT COUNT(*) FROM members WHERE status = ?");
    $stmt->execute([$filter]);
    $total = (int)$stmt->fetchColumn();

    $stmt = db()->prepare("SELECT * FROM members WHERE status = ? ORDER BY created_at DESC LIMIT {$per_page} OFFSET {$offset}");
    $stmt->execute([$filter]);
    $members = $stmt->fetchAll();
}

$counts = db()->query("SELECT status, COUNT(*) AS cnt FROM members GROUP BY status")->fetchAll(PDO::FETCH_KEY_PAIR);
    ...
    <div class="flex flex-wrap gap-2 text-xs">
        ...
        <a href="members<?= $key !== 'all' ? '?status=' . $key : '' ?>"
           class="px-3 py-1.5 rounded-lg font-medium transition-colors
                  <?= $filter === $key ? 'bg-[#1a1714] text-white' : 'bg-white border border-gray-200 text-gray-500 hover:border-gray-400' ?>">
            <?= $label ?> <span class="ml-1 <?= $filter === $key ? 'text-white/60' : 'text-gray-400' ?>">(<?= $count ?>)</span>
        </a>
        <?php endforeach; ?>
    </div>
</div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm min-w-[720px]">
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr><td colspan="8" class="px-5 py-10 text-center text-gray-400 text-sm">ไม่มีข้อมูล</td></tr>
                <?php endif; ?>
                <?php foreach ($members as $m): ?>
                <tr class="hover:bg-gray-50 <?= $m['status'] === 'pending' ? 'bg-amber-50/40' : '' ?>">
                    <td class="px-5 py-3 text-gray-400 text-xs"><?= $m['id'] ?></td>
                    <td class="px-5 py-3 font-medium text-gray-800 whitespace-nowrap"><?= htmlspecialchars($m['first_name'] . ' ' . $m['last_name']) ?></td>
                    ...
                    <td class="px-5 py-3 text-gray-500 text-xs whitespace-nowrap"><?= htmlspecialchars($m['phone'] ?? '—') ?></td>
                    <td class="px-5 py-3 text-gray-400 text-xs whitespace-nowrap"><?= date('d/m/Y H:i', strtotime($m['created_at'])) ?></td>
                    <td class="px-5 py-3">
                        <?php $badge = match($m['status']) {
